...
class GerarNotaController extends Controller
{
    public static function downloadNota($cdRcd)
    {
        if (GerarNotaController::verificaSeExisteNotaTelecom($cdRcd)) {
            ...
            return GerarNotaController::gerarNotaNfse($cdRcd);
        }
    }

    public static function gerarNotaDownload($cdrcd)
    {
        GerarNotaController::downloadNota($cdrcd);
        header('Content-Description: File Transfer');
        header('Content-Disposition: attachment; filename="Nota-' . $cdrcd . '.pdf');
        header('Content-Type: application/octet-stream');
        header('Content-Transfer-Encoding: binary');
        header('Content-Length: ' . filesize(storage_path("app/notas/Nota-$cdrcd.pdf")));
        header('Cache-Control: must-revalidate, post-check=0, pre-check=0');
        header('Pragma: public');
        header('Expires: 0');
        readfile(storage_path("app/notas/Nota-$cdrcd.pdf"));
    }

    public static function gerarNotaTelecom($cdRcd)
    {
        $nota =  NotaTelecom::getNota($cdRcd);
        $pdf = \PDF::loadView('notaTelecom', compact('nota'));
